...
export abstract class BaseShape2D implements IShape {
    ...
    protected drawLine( ctx : CanvasRenderingContext2D, style : string | CanvasGradient | CanvasPattern , isAxisX  = true ) {
        ctx.save();
    ...
    beginDraw(transformable: ITransformable, state: IRenderState, context: CanvasRenderingContext2D): void {
        ...
        const mat = transformable . getWorldMatrix();
    ...
    draw(transformable: ITransformable, state: IRenderState, context: CanvasRenderingContext2D): void {
        switch(state.renderType){
            case ERenderType.STROKE:
                context.stroke();
                break;
            case ERenderType.FILL:
                context.fill();
                break;
            case ERenderType.STROKE_FILL:
                context.stroke();
                context.fill();
                break;
            case ERenderType.CLIP:
                context.clip();
                break;
        }
    }
    ...
    endDraw(transformable: ITransformable, state: IRenderState, context: CanvasRenderingContext2D): void {
       if(state.renderType !== ERenderType.CLIP){
           if(state.showCoordSystem){
               ...
               this.drawLine(context,this.axisYStyle,true);
           }
           context.restore();
       }
    }
...
export class Circle extends BaseShape2D {
    ...
    public draw(transformable : ITransformable, state: IRenderState, context: CanvasRenderingContext2D) {
        context.beginPath();
        context.arc(0,0,this.radius,0.0,Math.PI * 2,true);
...
export class Ellipse extends BaseShape2D {
    ...
    public constructor ( radiusX  = 10 , radiusY  = 10 ) {
        super ( ) ;
    ...
    public hitTest ( localPt : vec2 , transform : ITransformable ): boolean {
        const isHitted : boolean = Math2D . isPointInEllipse ( localPt . x , localPt . y , 0 , 0 , this . radiusX, this . radiusY ) ;
        return isHitted;
    }
...
export class Rect extends BaseShape2D {
    ...
    public constructor ( w = 1, h = 1 , u  = 0 , v  = 0  ) {
        super ( ) ;
        this . width = w ;
        this . height = h ;
        this . x = - this . width * u ;
...
export class Grid extends Rect {
    ...
    public constructor ( w  = 10, h  = 10 , xStep  = 10, yStep  = 10 ) {
        super( w , h , 0 , 0 ) ;
        this . xStep = xStep ;
        this . yStep = yStep ;
    }
...
export class BezierPath extends BaseShape2D {
    ...
    public constructor ( points : vec2 [ ]  , isCubic  = false ) {
        super ( ) ;
        this . points = points ;
...
export class Line implements IShape {
    ...
    public beginDraw ( transformable : ITransformable , state : IRenderState , context : CanvasRenderingContext2D ): void {
        ...
        context . lineWidth = state . lineWidth ;
        context . strokeStyle = state . strokeStyle ;
        const mat : mat2d = transformable . getWorldMatrix ( ) ; 
        context . setTransform ( mat . values [ 0 ] , mat . values [ 1 ] , mat . values [ 2 ] , mat . values [ 3 ] , mat . values [ 4 ] , mat . values [ 5 ] ) ;
    }
