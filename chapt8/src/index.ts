import {  ISprite,  SpriteFactory, IShape,EOrder, ERenderType, Scale9Data  } from "./spriteSystem/interface";
import { CanvasMouseEvent, EInputEventType } from "./application";
import { vec2 , Math2D , Inset  } from "./math2d" ;
import { Canvas2DUtil } from "./Canvas2d/Canvas2DUtil" ;
import { Sprite2DApplication } from "./spriteSystem/sprite2DApplication" ;
...
class IShape_Event_Hittest_Draw_Test_Demo {
    ...
    private createSprites ( ) : void {
        const grid : IShape = SpriteFactory . createGrid ( this . _app . canvas . width , this . _app . canvas . height ) ;
        const gridSprite : ISprite = SpriteFactory . createSprite ( grid , 'grid' ) ;
        ...
        gridSprite . mouseEvent = ( s : ISprite , evt : CanvasMouseEvent ) : void => {
             ...
             if ( evt . button === 2 )
             {
                if ( evt . type === EInputEventType . MOUSEUP ) {
                    ...
                    const sprite : ISprite = SpriteFactory . createSprite (  this . _shapes [ this . _idx ] ) ;
                    ...
                    if ( this . _shapes [ this . _idx ] . type === 'Line' ) { 
                        ...
                        if ( this . _shapes [ this . _idx ] . type === 'Circle' ){ 
                            const scale : number = Math2D . random ( 1 , 3 ) ;
                            sprite . scaleX = scale ;
                            sprite . scaleY = scale ;
    ...
    private _createShapes ( ) : void {
        ...
        const triPoints : vec2 [ ] = [ vec2 . create ( 0 , 0 ) , vec2 . create ( 20 , 0 ) , vec2 . create ( 20 , 20 ) ] ;
        this . _shapes . push ( SpriteFactory . createPolygon ( triPoints ) ) ;

        const polyPoints : vec2 [ ] =  [ vec2 . create ( 20 , 0 ) ,
                   vec2 . create ( 10 , 20 ) ,
                   vec2 . create ( - 10 , 20 ) ,
                   vec2 . create ( - 20 , 0 ) ,
                   vec2 . create ( - 10 , - 20 ) ,
        ...
        const data : Scale9Data = new Scale9Data ( this . _image , new Inset ( 30 , 30 , 30 , 30  ) ) ;
...
const app : Sprite2DApplication = new Sprite2DApplication ( canvas , true ) ;
